...
def add_project(
    ...
    if project:
        return ProjectRead(
            ...
            documents=[DocumentInfo(id=doc.document_id, name=doc.name.split('/')[1]) for doc in project.documents]
        )
    else:
...
def get_projects(
    ...
    for proj in participated_projects:
        ...
        result.append(ProjectRead(
            ...
            documents=[DocumentInfo(id=doc.document_id, name=doc.name.split('/')[1]) for doc in documents]
        ))
